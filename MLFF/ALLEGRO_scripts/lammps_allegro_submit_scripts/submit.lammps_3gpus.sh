...
# 1 MPI rank x 3 GPUs (Kokkos -k on g 3). Uses in.npt_md_allegro.3gpus (processors 1 1 1).
# Do not override CUDA_VISIBLE_DEVICES; Slurm sets it for the allocated GPUs.
#SBATCH --partition=gpuquick
#SBATCH --ntasks=1
#SBATCH --gres=gpu:3
#SBATCH --nodes=1
#SBATCH -J lmp-allegro-3g
...
# Prefer 3-GPU input variant when present.
if [[ -f "./in.npt_md_allegro.3gpus" ]]; then
    INPUT_FILE="./in.npt_md_allegro.3gpus"
...
mpirun -np 1 \
    "$LMP_BINARY" -log none -k on g 3 -sf kk -pk kokkos newton on neigh half \
