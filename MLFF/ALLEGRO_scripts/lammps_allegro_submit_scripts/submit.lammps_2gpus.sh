...
# 2 MPI x 2 GPUs: in.npt_md_allegro with processors 2 1 1.
# (We previously tried gpucluster but it is unavailable right now.)
#SBATCH --ntasks=2
#SBATCH --gres=gpu:2
#SBATCH --nodes=1
...
mpirun -np 2 \
    "$LMP_BINARY" -log none -k on g 2 -sf kk -pk kokkos newton on neigh half \
